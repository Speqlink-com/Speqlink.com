...
import {
    ...
    TrendingUp, Bot, Briefcase, ChevronRight, Layers,
    DollarSign, BarChart2, Zap, Globe, ArrowRight
} from 'lucide-react';

const ProductLogo = ({ logo, name, className = '', imgClassName = '' }) => {
    return (
...
const products = [
    {
        ...
        features: ['Smart property discovery','Realtime availability','Hotel booking','Affordable housing access','Moving service hailing','Intelligent recommendations','Location-based filtering','Secure communication','Agent/property owner onboarding'],
    ...
    },
    {
        ...
        features: ['Quarry discovery','Constructor-to-driver matching','Delivery tracking','Intelligent logistics coordination','Supply optimization','Driver marketplace','Transaction monitoring','Realtime operational analytics'],
    ...
    },
    {
        ...
        features: ['Farmer marketplace','Agricultural ecommerce','Vendor onboarding','Logistics coordination','Agent stations in towns','Agricultural analytics','Smart delivery systems','Consumer-to-farmer connection'],
        ...
        switch (activeTab) {
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {product.features.map((f, i) => (
                                    <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                                        ...
                                        <CheckCircle className="w-4 h-4 text-[#38b6ff] flex-shrink-0" />
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {product.techStack.map((t, i) => (
                                    <span key={i} className="px-3 py-1 text-sm rounded-full bg-[#38b6ff]/10 text-[#38b6ff] border border-[#38b6ff]/20">{t}</span>
                                ))}
                            </div>
                        </div>
                    </div>
            ...
            case 'Business Model':
                return (
                    <div className="space-y-6">
                        {[
                            { label: 'Business Model', content: product.businessModel },
                            { label: 'Revenue Strategy', content: product.revenueStrategy },
                            { label: 'Scaling Potential', content: product.scalingPotential },
                        ].map((item) => (
                            <div key={item.label} className="p-6 rounded-xl bg-white/5 border border-white/10">
                                <h4 className="text-white font-semibold mb-3 flex items-center gap-2">
                                    <DollarSign className="w-4 h-4 text-[#38b6ff]" /> {item.label}
                                </h4>
                    ...
                    <div className="space-y-6">
                        <div className="p-6 rounded-xl bg-blue-500/10 border border-blue-500/20">
                            <h4 className="text-blue-400 font-semibold mb-3 flex items-center gap-2">
                                <TrendingUp className="w-5 h-5" /> Market Opportunity
                            </h4>
                            <p className="text-gray-300 leading-relaxed">{product.marketOpportunity}</p>
                        </div>
                        <div className="p-6 rounded-xl bg-white/5 border border-white/10">
                            <h4 className="text-white font-semibold mb-3 flex items-center gap-2">
                                <Globe className="w-4 h-4 text-[#38b6ff]" /> Future Vision
                            </h4>
                    ...
                    <div className="space-y-6">
                        <div className="p-6 rounded-xl bg-purple-500/10 border border-purple-500/20">
                            <h4 className="text-purple-400 font-semibold mb-3 flex items-center gap-2">
                                <Bot className="w-5 h-5" /> AI & Technology Advantage
                            </h4>
                            <p className="text-gray-300 leading-relaxed">{product.aiAdvantage}</p>
                        </div>
                        <div>
                            <h4 className="text-white font-semibold mb-4 flex items-center gap-2">
                                <Layers className="w-4 h-4 text-[#38b6ff]" /> Technology Stack
                            </h4>
                    ...
                    <div className="space-y-6">
                        <div className="p-6 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
                            <h4 className="text-yellow-400 font-semibold mb-3 flex items-center gap-2">
                                <Briefcase className="w-5 h-5" /> Investor Information
                            </h4>
                            <p className="text-gray-300 leading-relaxed">{product.investorNote}</p>
                        </div>
                        <div className="p-6 rounded-xl bg-white/5 border border-white/10">
                            <h4 className="text-white font-semibold mb-3 flex items-center gap-2">
                                <BarChart2 className="w-4 h-4 text-[#38b6ff]" /> Scaling Roadmap
                            </h4>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            <Link href="/invest">
                                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                    className="w-full sm:w-auto px-8 py-3 bg-gradient-to-r from-yellow-500 to-amber-500 text-black font-bold rounded-lg shadow-lg hover:shadow-xl transition-all flex items-center gap-2">
                                    <Briefcase className="w-4 h-4" /> Become an Investor
                                </motion.button>
                            </Link>
                            <Link href="/contact">
                                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            ...
            <section className="min-h-screen py-24 px-6 relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center mb-16">
                        ...
                        <h1 className="text-4xl md:text-6xl font-black text-white mb-6">Our Products</h1>
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-4 mb-12">
                        {products.map((p) => (
                            <motion.button key={p.id} onClick={() => { setActiveProduct(p); setActiveTab('Overview'); }}
                                ...
                                <div className="w-9 h-9 rounded-xl bg-white/10 overflow-hidden flex items-center justify-center flex-shrink-0 p-1">
                                    <Image
                                        src={p.logo}
                                        alt={`${p.name} logo`}
                                        width={36}
                                ...
                                <span className={`ml-auto text-xs px-2 py-1 rounded-full ${p.statusColor}`}>{p.status}</span>
                            </motion.button>
                        ))}
                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        <motion.div key={activeProduct.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}
                            ...
                            <div className={`p-8 md:p-12 bg-gradient-to-r ${activeProduct.color} relative overflow-hidden`}>
                                ...
                                <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-6">
                                    <div className="w-20 h-20 rounded-2xl bg-white/20 flex items-center justify-center flex-shrink-0 p-2 overflow-hidden">
                                        <Image
                                    ...
                                    <div>
                                        <div className="flex items-center gap-3 mb-2">
                                            <span className="text-xs font-semibold uppercase tracking-widest bg-white/20 px-3 py-1 rounded-full text-white">{activeProduct.category}</span>
                                            <span className={`text-xs px-2 py-1 rounded-full ${activeProduct.statusColor}`}>{activeProduct.status}</span>
                                        </div>
                                        <h2 className="text-4xl md:text-5xl font-black text-white mb-2">{activeProduct.name}</h2>
                                        <p className="text-white/80 text-xl font-light italic">{activeProduct.tagline}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="px-8 md:px-12 pt-6">
                                ...
                                <AnimatePresence mode="wait">
                                    <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }} className="pb-10">
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
                        ...
                        <div className="flex flex-col sm:flex-row justify-center gap-4">
                            <Link href="/invest">
                                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                    className="px-10 py-4 bg-gradient-to-r from-yellow-500 to-amber-500 text-black font-bold rounded-lg shadow-lg hover:shadow-xl transition-all flex items-center gap-2">
                                    <Briefcase className="w-5 h-5" /> Become an Investor
                                </motion.button>
                            </Link>
                            <Link href="/contact">
                                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
